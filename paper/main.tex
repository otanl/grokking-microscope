...
\title{What Governs Grokking in a Fully-Tractable Transformer?\\ A Controlled Study at 12K Parameters}
...
transition is gated by training-set coverage, and the coverage threshold is governed primarily by
the output cardinality (the modulus) and only secondarily by composition structure, an ordering
that replicates across a ten-fold change in domain size. (ii) Weight decay reproduces the Omnigrok
...
it), each flip a minority of seeds ($\approx$5/30 at the 0.70 threshold; the device count is
threshold-sensitive, 2--6/30) with no detectable change in the aggregate rate (paired-difference
95\% confidence intervals within $\pm20$ points). (iv) The mechanistic read-out is mixed:
...
\begin{enumerate}
\item \textbf{A coverage-cardinality regularity.} The grokking coverage threshold is governed
primarily by the output cardinality (the modulus $M$) rather than by the composition structure of
the task, an ordering that replicates across a ten-fold change in input-domain size
...
a minority of same-seed outcomes ($\approx$5/30 at the 0.70 threshold; the device count is
threshold-sensitive, 2--6/30) with no detectable aggregate bias, a fragility axis distinct from the
...
different in kind: a benign perturbation of the
reduction order or execution device that does not detectably change the average grok-rate but flips
individual seeds. Both point to grokking optimization operating near a numerical edge.
...
\citep{picard2021torch}, the precise hazard our multi-seed-rate protocol is built around. A related
...
$3\times10^{-3}$, batch size 16, weight decay 0.01), full-batch passes for up to 1{,}200--1{,}500
epochs, on CPU in float32. We define a run as having \emph{grokked} if its best held-out accuracy
...
to 1, 4, and 16, pairing by seed across 30 seeds.
...
outcome of \textbf{5 of 30 seeds (17\%)}. Seed 3 falls from 0.90 to 0.40 (loses grokking); seed 0
rises from 0.55 to 0.95 (gains it); seeds 11, 21, and 24 move comparably. The mean absolute change
in best accuracy is 0.14 and the maximum is 0.50. Yet the \emph{aggregate} grok-rate is
statistically indistinguishable: 7/30 at one thread versus 8/30 at four, with five discordant seeds
split 2 versus 3 (exact McNemar $p=1.0$). The perturbation shows \emph{no detectable bias}: it flips
which seeds generalize, not how many. We claim no \emph{detectable} bias rather than exact
unbiasedness, because the test is severely underpowered: with only five discordant pairs the
smallest two-sided McNemar $p$ the design can attain is 0.0625, so it cannot reach $p<0.05$
regardless of outcome, and a Newcombe paired-difference 95\% confidence interval on the rate still
spans $[-18, +12]$ percentage points. A larger seed budget could resolve a bias of that size.
...
right). Running $(a{+}b)\bmod 10$ at decay 0.1 paired by seed across 30 seeds, the two devices use
entirely different kernels and accumulation orders, yet the grok-rate is nearly identical (27/30 on
CPU, 26/30 on GPU) while 5 of 30 seeds flip (mean absolute change 0.11, again five discordant pairs
split 3 versus 2, exact McNemar $p=1.0$, Newcombe paired-difference 95\% CI $[-13, +20]$ points).
Two distinct perturbations of the floating-point environment, one (the device change) subsuming the
other (reduction order), produce a comparable signature: at the $\tau=0.7$ convention both flip
5 of 30 seeds with no detectable shift in the aggregate.
...
aggregate grok-rate near 25\%, the device comparison at decay 0.1 near 88\% -- yet both flip about
17\% of seeds, so the flip rate is not an artifact of poising the aggregate exactly at 50\%. We do
not, however, claim the two flip \emph{counts} are identical across thresholds: the thread count is
stable at 5/30 for $\tau\in\{0.6,0.7,0.8\}$, whereas the device count ranges 2--6/30 over the same
window (5/30 at the $\tau=0.7$ convention we report). What replicates is the qualitative signature --
...
4 CPU threads, 30 seeds; left) and execution device (CPU versus GPU, 30 seeds; right). Each point
is one seed; the only change between axes is the floating-point environment. Blue points keep their
grok status; orange diamonds cross the 0.70 grok threshold (flip). Flips occur at 5/30 for both
perturbations but straddle the diagonal symmetrically (exact McNemar $p=1.0$ each), so the aggregate
grok-rate shows no detectable change.}
\label{fig:knifeedge}
\end{figure}

The interpretation is that the generalizing solution is reachable but its basin is so shallow that
sub-ULP differences in how a sum is accumulated determine, for a non-trivial fraction of seeds,
whether optimization lands in it. This is benign on average and merely resamples the seed lottery,
unlike the directional Softmax Collapse of \citet{prieto2025edge}; either way, the numerical
environment is an uncontrolled variable that any single run silently fixes.
...
0.60, 0.70, and 0.80. The inverted-U, the thread flip count (5/30 at all three thresholds), and the
...
(2--6/30), but its McNemar test is non-significant at every threshold, so no conclusion is
...
All experiments use the public Glint-Research/Glimmer-1-Base checkpoint; all scripts and per-seed
records will be released, and every reported number -- including the paired statistics (exact
...
all sweeps on CPU in float32, with the device-control leg on a single NVIDIA RTX 3090 Ti.
